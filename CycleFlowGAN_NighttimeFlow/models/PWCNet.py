...
__all__ = [
    'pwc_dc_net', 'pwc_dc_net_old', 'get_pwcnet'
]
...
class PWCRainNet(nn.Module):

    def __init__(self, config, md=4):
        """
        input: md --- maximum displacement (for correlation. default: 4), after warpping

        """
        super(PWCRainNet, self).__init__()
    ...
    def warp(self, x, flo, rainmask1, rainmask2):
        """
        warp an image/tensor (img2) back to img1, according tot he optical flow
        However, we consider the rain streak location here
        :param x:
        :param rainmask:
        :param flo:
        :return:
        ...
        vgrid = vgrid.permute(0, 2, 3, 1)
        output = nn.functional.grid_sample(x, vgrid)
        mask = torch.autograd.Variable(torch.ones(x.size())).cuda(self.gpuid)
        mask = nn.functional.grid_sample(mask, vgrid)

        mask[mask < 0.9999] = 0
        mask[mask > 0] = 1
        mask[rainmask1 > 0.5] = 0
        mask[rainmask2 > 0.5] = 0
        return output * mask
    ...
    def forward(self, x):
        ...
        # Mask
        m11 = self.conv1n(self.conv1m(self.conv1mm(im1)))
        m21 = self.conv1n(self.conv1m(self.conv1mm(im2)))
        m12 = self.conv2n(self.conv2m(self.conv2mm(m11)))
        ...
        up_flow6 = self.deconv6(flow6)
        up_feat6 = self.upfeat6(x)
        rain6mask1 = self.masknet(x)
        rain6mask2 = self.masknet()

        warp5 = self.warp(c25, up_flow6 * 0.625, rainmask1, rainmask2)
        corr5 = self.corr(c15, warp5)
        corr5 = self.leakyRELU(corr5)
        x = torch.cat((corr5, c15, up_flow6, up_feat6), 1)
        x = torch.cat((self.conv5_0(x), x), 1)
